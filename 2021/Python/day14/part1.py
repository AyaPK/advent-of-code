...
it offers a polymer template and a list of pair insertion rules (your puzzle input). You just need to work out what
...
H occurs 161 times, and N occurs 865 times; taking the quantity of the most common element (B, 1749) and subtracting the quantity of the least common element
...
template = [x.strip() for x in open("input.txt").readlines()][0]
rules = {
    x.split(" -> ")[0]: x.split(" -> ")[1]
    for x in [x.strip() for x in open("input.txt").readlines()][2:]
}
...
result = template.count(most_common) - template.count(least_common)
print("Most common element:", most_common, "count:", template.count(most_common))
print("Least common element:", least_common, "count:", template.count(least_common))
print("Result (most - least):", result)
