...
"""
width = max([int(x.split(",")[0]) for x in open("input.txt").read().splitlines() if "," in x]) + 1
height = max([int(x.split(",")[1]) for x in open("input.txt").read().splitlines() if "," in x]) + 1

paper = [["." for _ in range(int(width))] for _ in range(int(height))]
...
for line in open("input.txt").read().splitlines():
    if "fold along" in line:
        ...
        if axis == "y":
            ...
            paper = [row[:value] for row in paper]
        break  # Only do the first fold
count = sum(row.count("#") for row in paper)
print(count)
